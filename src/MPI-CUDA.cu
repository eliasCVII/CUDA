 ...
 *
 */
__global__ void matrix_multiplication_kernel(float *A, float *B, float *C, int f, int c1, int c2) {
   int row;
   int col;
   int k;
   float sum;
...
int main(int argc, char *argv[]) {
   ...
   if (rank == MASTER) {
      ...
      while (workers > 0) {
         MPI_Recv(NULL, 0, MPI_INT, MPI_ANY_SOURCE, TAG_2, MPI_COMM_WORLD, &status);
      ...
      while (status.MPI_TAG != TAG_0) {
         ...
         if (strcmp(modo, SILENT) == 0) {
            printf("[Node %d]: %dx%d. CPU Time: %f s\n", rank, f, c2, (float) (ce - cs) / CLOCKS_PER_SEC);
         }
